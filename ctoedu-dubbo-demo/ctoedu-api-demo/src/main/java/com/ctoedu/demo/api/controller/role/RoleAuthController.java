...
public class RoleAuthController {
	...
	public @ResponseBody ViewerResult findMenuForAuthTree(HttpServletRequest request, @RequestBody JSONObject obj){
		...
		try {
			...
			JSONObject jsonObj = authService.find(currentUsername, appId, null, dataId, Principal.PRINCIPAL_ROLE, Resource.RESOURCE_MENU);
	...
	public @ResponseBody ViewerResult findAuthedMenu(HttpServletRequest request, @RequestBody JSONObject obj){
		...
		try {
			...
			if(currentUsername == null){
				...
				List<UmsAcl> aclList = umsAuthFacade.findAcl(roleId, Principal.PRINCIPAL_ROLE, Resource.RESOURCE_MENU);
	...
	public @ResponseBody ViewerResult addMenu(@RequestBody JSONObject obj){
		...
		try {
			...
			if(checked){
				umsAuthFacade.addPrincipalAcl(roleId, Principal.PRINCIPAL_ROLE, menuId, Resource.RESOURCE_MENU);
			}else{
				umsAuthFacade.deletePrincipalAcl(roleId, Principal.PRINCIPAL_ROLE, menuId, Resource.RESOURCE_MENU);
			}
			result.setSuccess(true);
		} catch (Exception e) {
			result.setSuccess(false);
	...
	public @ResponseBody ViewerResult addMenus(@RequestBody JSONObject obj){
		...
		try {
			...
			umsAuthFacade.addPrincipalAcl(roleId, Principal.PRINCIPAL_ROLE, menuIds, Resource.RESOURCE_MENU, null);
			result.setSuccess(true);
		} catch (Exception e) {
			result.setSuccess(false);
			result.setErrMessage(e.getMessage());
	...
	public @ResponseBody ViewerResult forbiddenMenu(@RequestBody JSONObject obj){
		...
		try {
			...
			boolean checked = obj.getBoolean("checked");
			if(checked){
				umsAuthFacade.forbiddenPrincipalAcl(dataId, Principal.PRINCIPAL_ROLE, menuId, Resource.RESOURCE_MENU);
			}else{
				umsAuthFacade.deleteForbiddenPrincipalAcl(dataId, Principal.PRINCIPAL_ROLE, menuId, Resource.RESOURCE_MENU);
			}
			result.setSuccess(true);
		} catch (Exception e) {
			result.setSuccess(false);
	...
	public @ResponseBody ViewerResult findControllerForAuthTree(HttpServletRequest request, @RequestBody JSONObject obj){
		...
		try {
			...
			String menuId = obj.getString("menuId");
			Object currentUsername = request.getAttribute("currentUsername");
			JSONObject jsonObj = authService.find(currentUsername, appId, menuId, dataId, Principal.PRINCIPAL_ROLE, Resource.RESOURCE_CONTROLLER);
			result.setSuccess(true);
			result.setData(jsonObj);
		} catch (Exception e) {
			result.setSuccess(false);
	...
	public @ResponseBody ViewerResult findAuthedController(HttpServletRequest request, @RequestBody JSONObject obj){
		...
		try {
			...
			if(currentUsername == null){
		        ...
		        String roleId = obj.getString("dataId");
				List<UmsAcl> aclList = umsAuthFacade.findAcl(roleId, Principal.PRINCIPAL_ROLE, Resource.RESOURCE_CONTROLLER);
				int size = aclList.size();
	...
	public @ResponseBody ViewerResult addControllerResource(@RequestBody JSONObject obj){
		...
		try {
			...
			boolean checked = obj.getBoolean("checked");
			if(checked){
				umsAuthFacade.addPrincipalAcl(roleId, Principal.PRINCIPAL_ROLE, controllerId, Resource.RESOURCE_CONTROLLER);
			}else{
				umsAuthFacade.deletePrincipalAcl(roleId, Principal.PRINCIPAL_ROLE, controllerId, Resource.RESOURCE_CONTROLLER);
			}
			result.setSuccess(true);
		} catch (Exception e) {
			result.setSuccess(false);
	...
	public @ResponseBody ViewerResult addControllerResources(@RequestBody JSONObject obj){
		...
		try {
			...
			umsAuthFacade.addPrincipalAcl(roleId, Principal.PRINCIPAL_ROLE, controllerIds, Resource.RESOURCE_CONTROLLER, null);
			result.setSuccess(true);
		} catch (Exception e) {
			result.setSuccess(false);
			result.setErrMessage(e.getMessage());
	...
	public @ResponseBody ViewerResult forbiddenController(@RequestBody JSONObject obj){
		...
		try {
			...
			boolean checked = obj.getBoolean("checked");
			if(checked){
				umsAuthFacade.forbiddenPrincipalAcl(dataId, Principal.PRINCIPAL_ROLE, controllerId, Resource.RESOURCE_CONTROLLER);
			}else{
				umsAuthFacade.deleteForbiddenPrincipalAcl(dataId, Principal.PRINCIPAL_ROLE, controllerId, Resource.RESOURCE_CONTROLLER);
			}
			result.setSuccess(true);
		} catch (Exception e) {
			result.setSuccess(false);
	...
	public @ResponseBody ViewerResult findAuthedResource(HttpServletRequest request, @RequestBody JSONObject obj){
		...
		try {
			...
			if(currentUsername == null){
				...
				String dataId = obj.getString("dataId");
				Map<String, Set<String>> resourcesMap = authedService.find(dataId, Principal.PRINCIPAL_ROLE);
